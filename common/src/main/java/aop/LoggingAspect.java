...
public class LoggingAspect {
    ...
    // 예외 발생 시 로깅
    @AfterThrowing(pointcut = "execution(* com.ticketPing.performance.application.service.*.*(..))",
        throwing = "ex")
    ...
    // 메서드 실행 이전 로깅
    @Before("execution(* com.ticketPing.performance.application.service.*.*(..))")
    ...
    // 메서드 실행 이후 로깅
    @After("execution(* com.ticketPing.performance.application.service.*.*(..))")
